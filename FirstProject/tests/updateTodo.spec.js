...
const createTodo= await request.post("http://localhost:5000/api/todo", {
  ...
  const updateTodo= await request.put('http://localhost:5000/api/todo/'+todoId, {
      headers: {
        Authorization: "Bearer "+token,
      },
    data: {
      title: "api create and update todo is done",
      description: "create and update todo is done",
    },
  });
  expect(response.status()).toBe(200);
  console.log( await updateTodo.json());
});
